...
\subsection{Interpretability as Architectural Property}

A striking consequence of the Hydra design is that its linguistic 
structure is \emph{not hidden}. Standard interpretability research works 
backwards: train a black box, then invest significant effort 
reverse-engineering what it learned via probing classifiers, sparse 
autoencoders, attention visualization, or causal tracing. Hydra inverts 
this workflow. The fiber annotations are literal tokens in the input 
stream---human-readable, inspectable, and editable before the model 
ever processes them.

This has practical consequences beyond transparency. A researcher can 
\emph{read} the model's input and see exactly what structural information 
was available. They can \emph{edit} a single fiber token (changing 
\texttt{[IS:TOPIC]} to \texttt{[IS:FOCUS]}, for instance) and observe 
how generation changes---obtaining causal interventions without 
activation patching or circuit analysis. They can \emph{compare} the 
explicit annotation (what the model was told) against the learned 
internal representation (what SAE features emerged)---which is precisely 
what our SAE experiment in Section~\ref{sec:sae} measures. And they 
can \emph{debug} generation failures by inspecting which fiber went 
wrong, rather than searching through thousands of hidden dimensions 
for the responsible circuit.

Our SAE results underscore this point. The model does not merely pass 
through the fiber tokens unchanged; it develops \emph{dedicated internal 
features} aligned with the explicit categories (POS features at MCC 
$0.80$, information structure at $0.44$). The annotation shapes the 
representation. This means the interpretability is not superficial---it 
propagates inward, organizing the model's internal geometry around 
human-legible axes rather than axes that must be discovered post hoc.

\subsection{Why SSMs Make This Practical}

The idea of enriching a token stream with structural metadata is not 
new---linguists have annotated corpora with parse trees, semantic roles, 
and discourse markers for decades. But injecting such annotations 
\emph{into the model's input} has been economically infeasible for 
transformers. Self-attention scales quadratically in sequence length: 
every fiber token added to the context makes \emph{every other token} 
more expensive to attend to. Doubling the token count (words plus 
structural tags) quadruples the attention cost and consumes half the 
finite context window with metadata rather than content. The economics 
are prohibitive, and this is why transformer-based approaches to 
linguistic structure have overwhelmingly favored \emph{implicit} 
representations---learned embeddings, auxiliary losses, structured 
attention masks---over explicit token-stream enrichment.

State-space models change this calculus fundamentally. Mamba processes 
sequences in \emph{linear} time with a fixed-size recurrent state and 
no context window to overflow. Doubling the token count doubles the 
compute, not quadruples it. And there is no window to saturate: the 
recurrent state carries information forward indefinitely, so fiber 
tokens injected early remain accessible throughout generation without 
displacing content tokens.

This is why Hydra is not simply ``linguistic preprocessing for any 
architecture'' but an approach whose practical viability depends on the 
SSM backbone. The fiber bundle framework is theoretically general---one 
could define the same fibers over a transformer---but the cost of 
manifesting those fibers as \emph{literal tokens in the stream} is 
affordable only when the backbone does not penalize sequence length 
quadratically. The more fibers we add (tone, rhythm, register, genre), 
the stronger this asymmetry becomes: each new fiber widens the gap 
between the SSM cost (linear in the number of new tokens) and the 
transformer cost (quadratic). Hydra is, in this sense, an architecture 
that \emph{could only have emerged} in the SSM era.

\subsection{Fiber Tokens as Generalized Chain of Thought}

Recent work on chain-of-thought (CoT) reasoning 
\citep{wei2022chain,nye2021show} has demonstrated that injecting 
intermediate reasoning tokens into the generation stream---even when 
those tokens are not part of the final output---dramatically improves 
model performance on complex tasks. We observe that Hydra's fiber 
annotations are doing something structurally identical: injecting 
explicit intermediate representations into the token stream to scaffold 
a cognitive process. The difference is not architectural but dimensional. 
CoT tags scaffold \emph{reasoning}; fiber tokens scaffold 
\emph{linguistic structure, affect, and discourse}.

This suggests a unifying view: all such injected tokens are 
\emph{fibers}. A reasoning trace wrapped in \texttt{<think>} tags is 
a reasoning fiber. A parse bracket \texttt{[PS:NP]} is a syntactic 
fiber. A rhetorical plan marker \texttt{[PLAN:compare-then-contrast]} 
would be a discourse fiber. An epistemic hedge 
\texttt{[CONFIDENCE:uncertain]} would be a calibration fiber. The token 
stream is a universal bus, and any dimension that aids downstream 
generation can be made explicit on it.

The SSM backbone makes this unification practical in a way that 
transformers cannot support. Chain-of-thought in transformers is 
\emph{expensive}: every thinking token inflates the quadratic attention 
cost, which has spawned an active literature on compressing reasoning 
traces, distilling long chains into short ones, and training models to 
think in fewer tokens \citep{deng2024explicit,goyal2024think}. This 
compression research exists precisely because transformers penalize 
stream length. With an SSM backbone, the cost of verbosity is linear. 
A model can simultaneously maintain reasoning tokens, linguistic fiber 
tokens, tonal markers, and rhetorical plan tags---``thinking out loud'' 
in multiple dimensions at once---without any fiber crowding out the 
others.

Moreover, Hydra already bridges the gap between \emph{pre-computed} 
annotations (produced by external heads before inference) and 
\emph{generated} annotations (produced by the model during 
autoregressive generation, as in CoT). During generation, the Hydra 
model produces both words and fiber tags autoregressively: it generates 
its own parse brackets, its own information-structure markers, its own 
structural scaffolding. Adding reasoning or planning tokens to the fiber 
vocabulary would simply introduce another dimension that the model 
learns to generate and consume in the same unified stream. The result 
would be a model that simultaneously plans its rhetorical move, sets 
its tonal register, scaffolds its syntax, and generates its words---all 
as explicit, interpretable, editable tokens.

This perspective extends beyond reasoning to what cognitive science 
calls \emph{executive function}: the maintenance of goals, constraints, 
and intentions that are temporally distant from the current token but 
essential to discourse coherence. A fiber token such as 
\texttt{[GOAL:build-toward-thesis-X]} represents goal maintenance; 
\texttt{[CONSTRAINT:reader-lacks-Y]} encodes audience modeling; 
\texttt{[CALLBACK:address-counterargument]} implements prospective 
memory---the intention to do something later. These are ``temporally 
inconvenient'' representations: they do not belong to any single token 
but must persist across many. In a transformer, such persistence 
requires the model to attend back to whichever earlier token established 
the goal---an implicit, fragile mechanism that degrades with distance. 
In an SSM, the recurrent state can carry forward-looking intentions 
indefinitely; making them explicit fiber tokens further ensures they 
are not merely carried but \emph{readable}, refreshable, and editable 
at any point in the generation process.

The resulting picture is of the token stream as a \emph{working memory 
buffer} with multiple concurrent registers: syntactic structure, 
semantic roles, tonal affect, rhetorical planning, and executive 
intentions, all co-present as interpretable tokens, all available to 
influence the next prediction, and all serializable for inspection or 
resumption. This is, we suggest, a more natural computational model of 
language production than the standard single-stream next-token 
prediction, and one that the SSM's linear cost makes practical to explore.

\paragraph{From annotation to computation.}
The logic of fiber enrichment extends beyond passive annotation to 
active computation. If the token stream can carry linguistic structure, 
tonal affect, and executive intentions, it can also carry 
\emph{knowledge} and \emph{queries}. A fact token such as 
\texttt{[FACT:ice-nine(fictional, destroys-water)]} grounds generation 
in retrieved knowledge---visible in the stream, auditable, editable. 
More radically, a query token such as 
\texttt{[QUERY:$\lambda x.\,\text{relevant}(x, \text{topic})$]} could 
represent a structured retrieval request that an external knowledge 
store executes mid-generation, with results re-injected as additional 
fiber tokens. This transforms the token stream from a passive sequence 
into a \emph{computation bus}: the model emits instructions alongside 
words, external systems execute them, and results flow back into the 
same stream that drives subsequent generation.

This is tool use at the \emph{token level} rather than the API-call 
level. Current tool-augmented language models interrupt generation, emit 
a function call (typically as opaque JSON), wait for a response, and 
resume. The fiber framework integrates this loop into the generation 
stream itself: queries and results are fiber tokens with the same 
interpretability guarantees as syntactic or tonal annotations. One can 
inspect exactly what the model asked, what was returned, and how it 
influenced the next word. If the query tokens are drawn from a 
compositional formalism such as the lambda calculus 
\citep{montague1973proper}, they combine in the same way that other 
fiber tokens combine---the model need not context-switch between 
``language mode'' and ``code mode'' because both are fibers on the 
same stream.

We do not claim to have implemented this vision, but we note that the 
mathematical and computational infrastructure is already in place: the 
fiber bundle framework provides the geometry, the SSM backbone provides 
the linear-cost processing, and the tag-interleaving fusion mechanism 
provides the interface. What remains is to train a model that learns 
to emit, consume, and act on computational fiber tokens---a direction 
we regard as among the most promising extensions of the Hydra framework.
...
At 39M parameters, Hydra achieves structural coherence---grammatically 
diverse, well-formed text---that vanilla Mamba models three times larger 
cannot produce. Yet semantic coherence remains limited: the model 
generates correct scaffolding but lacks the capacity to fill it with 
meaning. We hypothesize that structural offloading will \emph{compound} 
at larger scales. A 130M Hydra (matching GPT-2 Small's parameter budget) 
would retain the same structural heads, but the backbone---freed from 
rediscovering constituency, thematic roles, and information structure---could 
devote its full capacity to semantics. If this produces coherent, 
meaningful text at a parameter count where vanilla models cannot, 
it would constitute strong evidence that the path to efficient language 
modeling runs through geometric explicitness rather than brute-force scale.

\paragraph{Affective and prosodic fibers.}
Our four heads cover syntax, semantics, and discourse---but written text 
carries a further dimension that readers reconstruct implicitly: 
\emph{tone}. Sarcasm, formality, humor, gravity, and register are 
independent of parse structure (``That's just great'' has identical 
constituency whether sincere or sardonic) and thus constitute a genuine 
additional fiber. A tonal head could inject affect markers 
(e.g., \texttt{[TONE:sardonic]}, \texttt{[TONE:elevated]}) into the 
token stream, trained on emoji-rich social media where users 
\emph{already annotate their own prosody} via emoji. Stripping 
emoji from the text and predicting the original emoji cluster 
provides a natural supervised signal.

A related and perhaps more novel fiber is \emph{prosodic rhythm}: the 
discourse-level pacing of sentences. Good prose alternates short 
declarative sentences with long subordinate-clause-rich ones; legal 
writing punches facts in staccato then buries qualifications in 
baroque conditionals; stream-of-consciousness abandons conventional 
sentence boundaries entirely. These patterns are structural but 
operate \emph{above} the sentence level, in a dimension that no 
standard NLP annotation captures. Yet they are trivially computable 
from the surface text: sentence length in tokens, parse depth, 
clause count, and deviation from the local running average. Custom 
tokens such as \texttt{[LEN:short]}, \texttt{[RHYTHM:staccato]}, or 
\texttt{[STRUCT:inverted]} would give the model explicit coordinates 
for a dimension it would otherwise have to learn implicitly---or, at 
small scale, never learn at all.

More broadly, any extraneous metadata about a corpus---genre labels, 
publication venue, authorship, era---can be injected as additional 
fiber tokens. A sentence known to originate from a legal brief or 
a medical journal carries register information that need not be 
inferred from the words alone. The Hydra framework is not limited to 
linguistically \emph{standard} annotation schemes: any computable or 
known property of text defines a potential fiber. At generation time, 
a fusion layer can selectively strip or retain these tokens depending 
on the target venue, allowing the model to \emph{always reason with} 
tonal, rhythmic, and register information while \emph{optionally 
surfacing} it in the output. The fiber bundle framework accommodates 
this naturally: each new metadata source defines a new fiber, and 
the structure group determines how fibers interact during generation.
...
The continuation is structurally well-formed but semantically 
independent of the prompt---consistent with the trajectory 
dissociation discussed in Section~\ref{sec:discussion} and expected 
for a model of this scale. To put this in context, here is the same 
prompt processed by two comparison models:

\begin{itemize}
\item \textbf{GPT-2 Small} (124M parameters, $3\times$ larger, no 
annotations): \emph{``I'm sorry, I'm sorry,'' she said. ``I'm sorry. 
I'm sorry. I'm sorry. I'm sorry\ldots''} --- collapses into 
repetition within 15 tokens despite having $3\times$ Hydra's 
parameters.

\item \textbf{ps-only Mamba} (39M parameters, same architecture, 
phrase structure head only): generates a single word fragment 
(\emph{``Saprang''}) then collapses into character-level repetition 
(\emph{``ririririri\ldots''}) --- the fluency dissociation of 
Table~\ref{tab:results}.
\end{itemize}

\noindent
The three outputs illustrate three distinct failure modes: GPT-2 
achieves fluency but not diversity (repetition collapse); ps-only 
achieves neither (single-token dissociation); Hydra achieves both 
fluency and structural diversity but not semantic coherence with the 
prompt (trajectory dissociation). Each failure is a different 
symptom of the same underlying challenge: navigating the meaning 
manifold requires both knowing the coordinate system \emph{and} 
learning to move through it purposefully.
